 ...
 *  Changes:
 *
 *  v1.1.5 - 07/25/19 - Trying to get Members created within a virtual container 'Life360 Members'
...
def installed() {
    ...
    settings.users.each {memberId->
        ...
        if(member) {
            ...
            state.member = state.members.find{it.id==memberId}
...
def updated() {
    ...
    def container = getChildDevices().find{it.typeName == "Life360 Members"}
    def childDevices = childList()
    
    if(logEnable) log.debug "Child Devices = ${childDevices}"
    
    childDevices.each {childDevice->
        if(logEnable) log.debug "(l-441) Child = ${childDevice}, DNI=${childDevice.deviceNetworkId}"
...
def createContainer(){                    // Modified from @Stephack
    if(logEnable) log.info "In createContainer..."
	def container = getChildDevices().find{it.typeName == "Life360 Members"}
    if(logEnable) log.info "In createContainer - container: ${container}"
		if(!container){
			if(logEnable) log.info "Creating Life360 Members Virtual Container"
			try {
				container = addChildDevice("BPTWorld", "Life360 Members", "Life360.${app.id}", null, [name: "Life360 Members", label: "Life360 Members", completedSetup: true]) 
			} catch (e) {
				log.error "Life360 Members Container device creation failed with error = ${e}"
			}
        } else {
            if(logEnable) log.info "Life360 Members Container found!"
        }
}

def createVchild(){                    // Modified from @Stephack
    if(logEnable) log.info "In createVchild..."
	def container = getChildDevices().find{it.typeName == "Life360 Members"}
    if(logEnable) log.info "In createVchild - container: ${container}"
		if(container){
			def childDevice = container.addChildDevice("BPTWorld", "Life360 User", "L360.${state.member.id}",null,[name:state.member.firstName, completedSetup: true])
            if(logEnable) log.info "In createVchild - childDevice created - ${state.member.firstName} - childDevice=${childDevice}"
        } else {
            if(logEnable) log.info "In createVchild - No devices created - Members Container NOT found!"
        }
    return childDevice
}

def getContainerChildren(){            // Modified from @Stephack
    if(logEnable) log.info "In getContainerChildren..."
	def container = getChildDevices().find{it.typeName == "Life360 Members"}
    if(logEnable) log.info "In getContainerChildren - container: ${container}"
    def vChildren = container.childList()
    if(logEnable) log.info "In getContainerChildren - vChildren: ${vChildren}"
}

def childList(){                // Modified from @Stephack
    log.info "In childList..."
	def childDevice = getChildDevices()
    if(logEnable) log.info "In childList - children: ${children}"
	//return children
    return childDevice
}


// *** another attempt to get child devices grouped ***
/**
def createChildDevices() {
    state.oldLabel = state.member.label
    
    def childDevice = addChildDevice("BPTWorld", "Life360 User", "L360.${state.member.id}", [completedSetup: true, label: "${state.member.firstName})"])
      //addChildDevice("Life360 User", "L360.${state.member.id}",null,[name:state.member.firstName, completedSetup: true])
