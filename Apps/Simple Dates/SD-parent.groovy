 ...
 *  V1.0.0 - 06/04/19 - Initial release.
 *
 */
...
def setVersion(){
    ...
    state.appName = "SimpleDatesParentVersion"
    ...
    dynamicPage(name: "mainPage") {
		...
		if(state.appInstalled == 'COMPLETE'){
			section(getFormat("title", "${app.label}")) {
				paragraph "<div style='color:#1A77C9'>Create a simple coutdown to your most important dates.</div>"
				paragraph getFormat("line")
			}
			section("Instructions:", hideable: true, hidden: true) {
				paragraph "<b>Information</b>"
				paragraph "Create a simple coutdown to your most important dates."
			}
			section(getFormat("header-green", "${getImage("Blank")}"+" Child Apps")) {
				app(name: "anyOpenApp", appName: "Simple Dates Child", namespace: "BPTWorld", title: "<b>Add a new 'Simple Dates' child</b>", multiple: true)
			}
            // ** App Watchdog Code **
