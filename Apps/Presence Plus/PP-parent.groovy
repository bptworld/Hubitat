...
def setVersion(){
    ...
    state.appName = "PresencePlusParentVersion"
    ...
    dynamicPage(name: "mainPage") {
		...
		if(state.appInstalled == 'COMPLETE'){
			section(getFormat("title", "${app.label}")) {
				paragraph "<div style='color:#1A77C9'>Creates a combined presence device that can be used with Life360 Tracker, Google Assistant, Rule Machine and More!</div>"
			...
			section(getFormat("header-green", "${getImage("Blank")}"+" Child Apps")) {
				app(name: "anyOpenApp", appName: "Presence Plus Child", namespace: "BPTWorld", title: "<b>Add a new 'Presence Plus' child</b>", multiple: true)
			}
            // ** App Watchdog Code **
