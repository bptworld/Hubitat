/**
 *  ****************  Abacus Counting Machine Driver  ****************
 *
 *  Design Usage:
 *  This driver formats the Abacus Counting Machine data to be used with Hubitat's Dashboards.
 *
...
def sendContactMap(data) {
    if(logEnable) log.debug "In Abacus Counting Machine Driver - Contact - Received new data!"
    def (whichMap, theData) = data.split('::')
	count = theData.length()
	if(count <= 1024) {
		if(logEnable) log.debug "In Abacus Counting Machine Driver - Contact - Map has ${count} Characters"
	} else {
...
def sendMotionMap(data) {
    if(logEnable) log.debug "In Abacus Counting Machine Driver - Motion - Received new data!"
    def (whichMap, theData) = data.split('::')
	count = theData.length()
	if(count <= 1024) {
		if(logEnable) log.debug "In Abacus Counting Machine Driver - Motion - Map has ${count} Characters"
	} else {
		theData = "Too many characters to display on Dashboard (${count})"
	}
    if(whichMap == "1") {
...
def sendSwitchMap(data) {
    if(logEnable) log.debug "In Abacus Counting Machine Driver - Switch - Received new data!"
    def (whichMap, theData) = data.split('::')
	count = theData.length()
	if(count <= 1024) {
		if(logEnable) log.debug "In Abacus Counting Machine Driver - Switch - Map has ${count} Characters"
	} else {
		theData = "Too many characters to display on Dashboard (${count})"
	}
    if(whichMap == "1") {
...
def sendThermostatMap(data) {
    if(logEnable) log.debug "In Abacus Counting Machine Driver - Thermostat - Received new data!"
    def (whichMap, theData) = data.split('::')
	count = theData.length()
	if(count <= 1024) {
		if(logEnable) log.debug "In Abacus Counting Machine Driver - Thermostat - Map has ${count} Characters"
	} else {
		theData = "Too many characters to display on Dashboard (${count})"
	}
    if(whichMap == "1") {
