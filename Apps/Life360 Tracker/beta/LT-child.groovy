...
def trackingHandler(evt) {
    if(logEnable) log.debug "In trackingHandler (${state.version})"
    getTimeDiff()
    ...
    if(logEnable) log.debug "In trackingHandler - ${friendlyName} is at ${state.address1Value}"
    if(state.address1Value == state.prevPlace) {
        if(state.tDiff >= timeHere) {
            if(logEnable) log.debug "In trackingHandler - Time at Place: ${state.tDiff} IS greater than: ${timeHere} - beenHere: ${state.beenHere}"
            if(state.beenHere == "no") {
                if(logEnable) log.debug "In trackingHandler - ${friendlyName} has arrived at ${state.address1Value}"
                if(trackingOptions == "Track Specific") {
                    if(logEnable) log.debug "In trackingHandler - using Track Specific"
                    if(trackSpecific.contains(state.address1Value)) {
                        state.msg = "${messageAT}"
                        state.speakAT = "yes"
                    }
                } else {        
                    if(logEnable) log.debug "In trackingHandler - using Track All"
                    state.msg = "${messageAT}"
                    state.speakAT = "yes"
                }
                if(state.msg) messageHandler()
            } else {
                if(logEnable) log.debug "In trackingHandler - ${friendlyName} has been at ${state.address1Value} for ${state.timeDay} days, ${state.timeHrs} hrs, ${state.timeMin} mins & ${state.timeSec} secs - beenHere: ${state.beenHere}"
            }
            if(isDataDevice) isDataDevice.on()
            state.beenHere = "yes"
            if(logEnable) log.debug "In trackingHandler - state.tDiff > timeHere - TRUE - beenHere: ${state.beenHere}"
        } else {  // ***  state.tDiff is NOT GREATER THAN timeHere ***
            if(logEnable) log.debug "In trackingHandler - Time at Place: ${state.tDiff} IS NOT greater than: ${timeHere} - beenHere: ${state.beenHere}"
            if(isDataDevice) isDataDevice.off()
            state.beenHere = "no"
            if(logEnable) log.debug "In trackingHandler - state.tDiff > timeHere - FALSE - beenHere: ${state.beenHere}"
        }
    } else {  // *** state.address1Value != state.prevPlace ***
        if(logEnable) log.debug "In trackingHandler - address1: ${state.address1Value} DOES NOT MATCH state.prevPlace: ${state.prevPlace} - beenHere: ${state.beenHere}"
        if(state.beenHere == "yes") {
            if(logEnable) log.debug "In trackingHandler - ${friendlyName} has departed from ${state.prevPlace}"
            if(trackingOptions == "Track Specific") {
                if(trackSpecific.contains(state.prevPlace)) {
                    if(logEnable) log.debug "In trackingHandler - using Track Specific"
            ...
            if(state.sMove == null) {
                ...
                if(logEnable) log.debug "In trackingHandler - Time Moving: ${now} - sMove: ${state.sMove}"
            }
            if(state.justDEP == "no") {
                getTimeMoving()
                if(state.mDiff >= timeMoving) {
                    if(logEnable) log.debug "In trackingHandler - ${friendlyName} is on the move near ${state.address1Value}"
                    ...
                    if(logEnable) log.debug "In trackingHandler - ${friendlyName} has been on the move less than ${timeMove} minutes but is near ${state.address1Value}"   
                }
            } else {
                state.justDEP = "no"
                if(logEnable) log.debug "In trackingHandler - ${friendlyName} has just departed so skipping first move notice, but is near ${state.address1Value}"
            }
        }       
        if(isDataDevice) isDataDevice.off()
        state.beenHere = "no"
        if(logEnable) log.debug "In trackingHandler (${state.version}) - End of Departed/Move - beenHere: ${state.beenHere}"
    }
...
def messageHandler() {
    ...
    if(theMessage.contains("%lastplace%")) {theMessage = theMessage.replace('%lastplace%', state.prevPlace )}
