 ...
 * ------------------------------------------------------------------------------------------------------------------------------
 *
 *  1.0.8 - 05/15/22 - App: Minor cosmetic changes, Driver: Adjustment to sendDup and goodKeywords now checks Name and Message
 *  ---
...
def pageConfig() {
    dynamicPage(name: "", title: "", install: true, uninstall: true) {
		...
		section(getFormat("header-green", "${getImage("Blank")}"+" Notification Options")) {
            ...
            input "sendDup", "bool", title: "Send push even if the Error was the same as the last Error", defaultValue:false, submitOnChange:true
            paragraph "<hr>"
        ...
        section(getFormat("header-green", "${getImage("Blank")}"+" Other Options")) {
            input "goodKeywords", "text", title: "Ignore error if any of these keywords are found in the Name or Message (seperate multiple by ; (semicolon))", required:false, submitOnChange:true
            paragraph ""
...
def initialize() {
    ...
    if(pauseApp || state.eSwitch) {
        ...
        if(dataDevice) {
            dataDevice.sendEvent(name: "goodKeywords", value: goodKeywords, isStateChange: true)
