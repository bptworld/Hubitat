		...
		if(state.appInstalled == 'COMPLETE'){
            ...
            section(getFormat("header-green", "${getImage("Blank")}"+" Global Config")) {
                ...
                if(makerName || makerURL) {
                    href "urlOptions", title:"${getImage("optionsGreen")} Shorten URLs", description:"Click here for Options"
                } else {
                    href "urlOptions", title:"${getImage("optionsRed")} Shorten URLs", description:"Click here for Options"
                }
            }
        }
    }
...
def urlOptions() {
    dynamicPage(name: "urlOptions", title: "", install:false, uninstall:false) {
        display()
        section(getFormat("header-green", "${getImage("Blank")}"+" URL Options")) {}
        section("Instructions:", hideable: true, hidden: true) {
			paragraph "<b>Shorten the Maker URLs without using an external source!</b>"
            
            instruct =  "It is highly recommended to shorten the Maker URLs to save on character count.<br>"
            instruct += "Previouly this was done with an external source (bitly), now it can be done right here in the app LOCALLY!"
