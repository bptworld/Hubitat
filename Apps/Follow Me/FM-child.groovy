...
def presenceSensorHandler(evt) {
    ...
    if(pauseApp || state.eSwitch) {
        ...
        if(state.presenceStatus == "present") {
            ...
            if(lastActive == null) lastActive = false
            speakerStatus = "${app.label}:${atomicState.sZone}:${app.id}:${lastActive}"
            if(logEnable) log.debug "In presenceSensorHandler - ***** speakerStatus: ${speakerStatus} *****"
            gvDevice.sendFollowMeSpeaker(speakerStatus)
        ...
        if(state.presenceStatus == "not present" && !lastActive) {
            ...
            if(logEnable) log.debug "In presenceSensorHandler - lastActive is True"
        }
    }
}
...
def zoneOffHandler(data = null) {
    ...
    if(notActive) {
        ...
        if(state.contactStatus == "open" || state.motionStatus == "active" || state.switchStatus == "on" || state.presenceStatus == "present"){
            atomicState.sZone = true
...
def uninstalled() { // library marker BPTWorld.bpt-normalStuff, line 64
    sendLocationEvent(name: "updateVersionInfo", value: "${app.id}:remove") // library marker BPTWorld.bpt-normalStuff, line 65
	removeChildDevices(getChildDevices()) // library marker BPTWorld.bpt-normalStuff, line 66
} // library marker BPTWorld.bpt-normalStuff, line 67

private removeChildDevices(delete) { // library marker BPTWorld.bpt-normalStuff, line 69
	delete.each {deleteChildDevice(it.deviceNetworkId)} // library marker BPTWorld.bpt-normalStuff, line 70
} // library marker BPTWorld.bpt-normalStuff, line 71

def letsTalk(msg) { // library marker BPTWorld.bpt-normalStuff, line 73
    if(logEnable) log.debug "In letsTalk (${state.version}) - Sending the message to Follow Me - msg: ${msg}" // library marker BPTWorld.bpt-normalStuff, line 74
    if(useSpeech && fmSpeaker) { // library marker BPTWorld.bpt-normalStuff, line 75
        fmSpeaker.latestMessageFrom(state.name) // library marker BPTWorld.bpt-normalStuff, line 76
        fmSpeaker.speak(msg,null) // library marker BPTWorld.bpt-normalStuff, line 77
    } // library marker BPTWorld.bpt-normalStuff, line 78
} // library marker BPTWorld.bpt-normalStuff, line 79

def pushHandler(msg){ // library marker BPTWorld.bpt-normalStuff, line 81
    if(logEnable) log.debug "In pushNow (${state.version}) - Sending a push - msg: ${msg}" // library marker BPTWorld.bpt-normalStuff, line 82
    theMessage = "${app.label} - ${msg}" // library marker BPTWorld.bpt-normalStuff, line 83
    if(logEnable) log.debug "In pushNow - Sending message: ${theMessage}" // library marker BPTWorld.bpt-normalStuff, line 84
    sendPushMessage.deviceNotification(theMessage) // library marker BPTWorld.bpt-normalStuff, line 85
} // library marker BPTWorld.bpt-normalStuff, line 86

// ********** Normal Stuff ********** // library marker BPTWorld.bpt-normalStuff, line 88
def logsOff() { // library marker BPTWorld.bpt-normalStuff, line 89
    log.info "${app.label} - Debug logging auto disabled" // library marker BPTWorld.bpt-normalStuff, line 90
    app.updateSetting("logEnable",[value:"false",type:"bool"]) // library marker BPTWorld.bpt-normalStuff, line 91
} // library marker BPTWorld.bpt-normalStuff, line 92

def checkEnableHandler() { // library marker BPTWorld.bpt-normalStuff, line 94
    state.eSwitch = false // library marker BPTWorld.bpt-normalStuff, line 95
    if(disableSwitch) {  // library marker BPTWorld.bpt-normalStuff, line 96
        if(logEnable) log.debug "In checkEnableHandler - disableSwitch: ${disableSwitch}" // library marker BPTWorld.bpt-normalStuff, line 97
        disableSwitch.each { it -> // library marker BPTWorld.bpt-normalStuff, line 98
            theStatus = it.currentValue("switch") // library marker BPTWorld.bpt-normalStuff, line 99
            if(theStatus == "on") { state.eSwitch = true } // library marker BPTWorld.bpt-normalStuff, line 100
        } // library marker BPTWorld.bpt-normalStuff, line 101
        if(logEnable) log.debug "In checkEnableHandler - eSwitch: ${state.eSwitch}" // library marker BPTWorld.bpt-normalStuff, line 102
    } // library marker BPTWorld.bpt-normalStuff, line 103
} // library marker BPTWorld.bpt-normalStuff, line 104

def getImage(type) {					// Modified from @Stephack Code // library marker BPTWorld.bpt-normalStuff, line 106
    def loc = "<img src=https://raw.githubusercontent.com/bptworld/Hubitat/master/resources/images/" // library marker BPTWorld.bpt-normalStuff, line 107
    if(type == "Blank") return "${loc}blank.png height=40 width=5}>" // library marker BPTWorld.bpt-normalStuff, line 108
    if(type == "checkMarkGreen") return "${loc}checkMarkGreen2.png height=30 width=30>" // library marker BPTWorld.bpt-normalStuff, line 109
    if(type == "optionsGreen") return "${loc}options-green.png height=30 width=30>" // library marker BPTWorld.bpt-normalStuff, line 110
    if(type == "optionsRed") return "${loc}options-red.png height=30 width=30>" // library marker BPTWorld.bpt-normalStuff, line 111
    if(type == "instructions") return "${loc}instructions.png height=30 width=30>" // library marker BPTWorld.bpt-normalStuff, line 112
    if(type == "logo") return "${loc}logo.png height=60>" // library marker BPTWorld.bpt-normalStuff, line 113
} // library marker BPTWorld.bpt-normalStuff, line 114

def getFormat(type, myText="") {			// Modified from @Stephack Code // library marker BPTWorld.bpt-normalStuff, line 116
    if(type == "header-green") return "<div style='color:#ffffff;font-weight: bold;background-color:#81BC00;border: 1px solid;box-shadow: 2px 3px #A9A9A9'>${myText}</div>" // library marker BPTWorld.bpt-normalStuff, line 117
    if(type == "line") return "<hr style='background-color:#1A77C9; height: 1px; border: 0;'>" // library marker BPTWorld.bpt-normalStuff, line 118
    if(type == "title") return "<h2 style='color:#1A77C9;font-weight: bold'>${myText}</h2>" // library marker BPTWorld.bpt-normalStuff, line 119
} // library marker BPTWorld.bpt-normalStuff, line 120

def display(data) { // library marker BPTWorld.bpt-normalStuff, line 122
    if(data == null) data = "" // library marker BPTWorld.bpt-normalStuff, line 123
    setVersion() // library marker BPTWorld.bpt-normalStuff, line 124
    getHeaderAndFooter() // library marker BPTWorld.bpt-normalStuff, line 125
    if(app.label) { // library marker BPTWorld.bpt-normalStuff, line 126
        if(app.label.contains("(Paused)")) { // library marker BPTWorld.bpt-normalStuff, line 127
            theName = app.label - " <span style='color:red'>(Paused)</span>" // library marker BPTWorld.bpt-normalStuff, line 128
        } else { // library marker BPTWorld.bpt-normalStuff, line 129
            theName = app.label // library marker BPTWorld.bpt-normalStuff, line 130
        } // library marker BPTWorld.bpt-normalStuff, line 131
    } // library marker BPTWorld.bpt-normalStuff, line 132
    if(theName == null || theName == "") theName = "New Child App" // library marker BPTWorld.bpt-normalStuff, line 133
    section (getFormat("title", "${getImage("logo")}" + " ${state.name} - ${theName}")) { // library marker BPTWorld.bpt-normalStuff, line 134
        paragraph "${state.headerMessage}" // library marker BPTWorld.bpt-normalStuff, line 135
        paragraph getFormat("line") // library marker BPTWorld.bpt-normalStuff, line 136
        input "pauseApp", "bool", title: "Pause App", defaultValue:false, submitOnChange:true // library marker BPTWorld.bpt-normalStuff, line 137
    } // library marker BPTWorld.bpt-normalStuff, line 138
} // library marker BPTWorld.bpt-normalStuff, line 139

def display2() { // library marker BPTWorld.bpt-normalStuff, line 141
    section() { // library marker BPTWorld.bpt-normalStuff, line 142
        if(state.appType == "parent") { href "removePage", title:"${getImage("optionsRed")} <b>Remove App and all child apps</b>", description:"" } // library marker BPTWorld.bpt-normalStuff, line 143
        paragraph getFormat("line") // library marker BPTWorld.bpt-normalStuff, line 144
        paragraph "<div style='color:#1A77C9;text-align:center;font-size:20px;font-weight:bold'>${state.name} - ${state.version}</div>" // library marker BPTWorld.bpt-normalStuff, line 145
        paragraph "${state.footerMessage}" // library marker BPTWorld.bpt-normalStuff, line 146
    } // library marker BPTWorld.bpt-normalStuff, line 147
} // library marker BPTWorld.bpt-normalStuff, line 148

def getHeaderAndFooter() { // library marker BPTWorld.bpt-normalStuff, line 150
    timeSinceNewHeaders() // library marker BPTWorld.bpt-normalStuff, line 151
    if(state.checkNow == null) state.checkNow = true // library marker BPTWorld.bpt-normalStuff, line 152
    if(state.totalHours > 6 || state.checkNow) { // library marker BPTWorld.bpt-normalStuff, line 153
        def params = [ // library marker BPTWorld.bpt-normalStuff, line 154
            uri: "https://raw.githubusercontent.com/bptworld/Hubitat/master/info.json", // library marker BPTWorld.bpt-normalStuff, line 155
            requestContentType: "application/json", // library marker BPTWorld.bpt-normalStuff, line 156
            contentType: "application/json", // library marker BPTWorld.bpt-normalStuff, line 157
            timeout: 10 // library marker BPTWorld.bpt-normalStuff, line 158
        ] // library marker BPTWorld.bpt-normalStuff, line 159
        try { // library marker BPTWorld.bpt-normalStuff, line 160
            def result = null // library marker BPTWorld.bpt-normalStuff, line 161
            httpGet(params) { resp -> // library marker BPTWorld.bpt-normalStuff, line 162
                state.headerMessage = resp.data.headerMessage // library marker BPTWorld.bpt-normalStuff, line 163
                state.footerMessage = resp.data.footerMessage // library marker BPTWorld.bpt-normalStuff, line 164
            } // library marker BPTWorld.bpt-normalStuff, line 165
        } catch (e) { } // library marker BPTWorld.bpt-normalStuff, line 166
    } // library marker BPTWorld.bpt-normalStuff, line 167
    if(state.headerMessage == null) state.headerMessage = "<div style='color:#1A77C9'><a href='https://github.com/bptworld/Hubitat' target='_blank'>BPTWorld Apps and Drivers</a></div>" // library marker BPTWorld.bpt-normalStuff, line 168
...
def timeSinceNewHeaders() {  // library marker BPTWorld.bpt-normalStuff, line 172
    if(state.previous == null) {  // library marker BPTWorld.bpt-normalStuff, line 173
        prev = new Date() // library marker BPTWorld.bpt-normalStuff, line 174
    } else { // library marker BPTWorld.bpt-normalStuff, line 175
        try { // library marker BPTWorld.bpt-normalStuff, line 176
            SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ssZ") // library marker BPTWorld.bpt-normalStuff, line 177
            prev = dateFormat.parse("${state.previous}".replace("+00:00","+0000")) // library marker BPTWorld.bpt-normalStuff, line 178
        } catch(e) { // library marker BPTWorld.bpt-normalStuff, line 179
            prev = state.previous // library marker BPTWorld.bpt-normalStuff, line 180
        } // library marker BPTWorld.bpt-normalStuff, line 181
    } // library marker BPTWorld.bpt-normalStuff, line 182
    def now = new Date() // library marker BPTWorld.bpt-normalStuff, line 183
    use(TimeCategory) { // library marker BPTWorld.bpt-normalStuff, line 184
        state.dur = now - prev // library marker BPTWorld.bpt-normalStuff, line 185
        state.days = state.dur.days // library marker BPTWorld.bpt-normalStuff, line 186
        state.hours = state.dur.hours // library marker BPTWorld.bpt-normalStuff, line 187
        state.totalHours = (state.days * 24) + state.hours // library marker BPTWorld.bpt-normalStuff, line 188
    } // library marker BPTWorld.bpt-normalStuff, line 189
    state.previous = now // library marker BPTWorld.bpt-normalStuff, line 190
